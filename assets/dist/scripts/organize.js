...
function updateControls(table) {
  table.children('tbody').children('tr').each(function (i, el) {
    var controls = '';
    var up = "<button class=\"move-up\">".concat(PB_OrganizeToken.moveUp, "</button>");
    var down = "<button class=\"move-down\">".concat(PB_OrganizeToken.moveDown, "</button>");

    if ($(el).is('tr:only-of-type')) {
      if (table.is('[id^=part]') && table.prevAll('[id^=part]').length && table.nextAll('[id^=part]').length) {
...
$(document).ready(function () {
  ...
  $(document).on('click', '.move-up', function (event) {
    ...
    if (row.is('tr:first-of-type') && table.is('[id^=part]') && table.prevAll('[id^=part]').length) {
  ...
  $(document).on('click', '.move-down', function (event) {
    ...
    if (row.is('tr:last-of-type') && table.is('[id^=part]') && table.nextAll('[id^=part]').length) {
  ...
  $('.allow-bulk-operations table thead th button[id$="show_title"]').on('click', function (event) {
  ...
  $('.allow-bulk-operations table thead th button[id$="visibility"]').on('click', function (event) {
    var id = $(event.target).attr('id');
    id = id.replace('-', '');
