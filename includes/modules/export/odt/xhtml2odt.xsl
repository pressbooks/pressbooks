				...
				<office:automatic-styles>
					...
					<style:page-layout style:name="Mpm2">
						<style:page-layout-properties
							...
							style:writing-mode="lr-tb" style:footnote-max-height="0cm">
							<style:footnote-sep style:width="0.018cm"
								style:adjustment="left" style:rel-width="33%" style:color="#000000" />
						</style:page-layout-properties>
						<style:header-style>
							<style:header-footer-properties
								fo:min-height="0cm" fo:margin-bottom="0.499cm" />
		...
		<xsl:choose>
			...
			<xsl:when test="@class='textbox'">
				<text:p text:style-name="Standard">
					<draw:frame draw:style-name="imageFrame1" text:anchor-type="paragraph" svg:width="6.9252in" draw:z-index="0">
	...
	<xsl:template match="li">
		<text:list-item>
			<xsl:choose>
				...
				<xsl:when test="./(ul|ol) and not(descendant::*[matches(local-name(), '^(p|h[1-6])$')])"><!-- span -->
					<text:p>
				...
				<xsl:when test="descendant::*[matches(local-name(), '^(p|h[1-6])$')]"><!-- span -->
					<xsl:apply-templates/>
				</xsl:when>
				<xsl:otherwise>
					<text:p>
	...
	<xsl:template match="img">
		<xsl:choose>
			<xsl:when test="parent::p">
				<draw:frame draw:style-name="imageFrame1" text:anchor-type="paragraph" svg:x="0in" svg:y="0in" svg:width="6.26806in"
	...
	<xsl:template match="a">
		<xsl:choose>
			...
			<xsl:when test="starts-with(@href, '#') and ancestor::div[@id='toc']">
				<text:a xlink:type="simple" xlink:href="{@href}" text:style-name="Internet_link" text:visited-style-name="Visited_Internet_Link">
					<xsl:apply-templates/>
				</text:a>
			</xsl:when>
