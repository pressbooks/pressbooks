	...
	protected function echoFrontMatter( $book_contents, $metadata ) {
		...
		foreach ( $book_contents['front-matter'] as $front_matter ) {
			...
			printf(
				...
				$subclass,
				$slug,
				( $short_title ) ? $short_title : wp_strip_all_tags( html_entity_decode( $front_matter['post_title'] ) ),
				$i,
				Sanitize\decode( $title ),
				$after_title,
				$content,
		...
		foreach ( $book_contents['part'] as $part ) {
			...
			foreach ( $part['chapters'] as $chapter ) {
				...
				$my_chapters .= sprintf(
					...
					$subclass,
					$slug,
					( $short_title ) ? $short_title : wp_strip_all_tags( html_entity_decode( $chapter['post_title'] ) ),
					$my_chapter_number,
					Sanitize\decode( $title ),
					$after_title,
					'<pre>' . wp_strip_all_tags( html_entity_decode( $chapter['post_title'] ) ) . '</pre>' . $content,
	...
	protected function echoBackMatter( $book_contents, $metadata ) {
		...
		foreach ( $book_contents['back-matter'] as $back_matter ) {
			...
			printf(
				...
				$subclass,
				$slug,
				( $short_title ) ? $short_title : wp_strip_all_tags( html_entity_decode( $back_matter['post_title'] ) ),
				$i,
				Sanitize\decode( $title ),
				$after_title,
				$content,
