...
class Posts extends \WP_REST_Posts_Controller {
	...
	public function overrideResponse( $response, $post, $request ) {
		...
		if ( $post->post_type === 'part' && isset( $response->data['meta'] ) ) {
			$response->data['meta']['pb_part_invisible_string'] = ! isset( $response->data['meta']['pb_part_invisible'] ) || is_null( $response->data['meta']['pb_part_invisible'] ) ?
				'on' : '';
		}
	...
	public function get_items_permissions_check( $request ): bool | \WP_Error {
		...
		return current_user_can( 'edit_posts' ) || get_option( 'blog_public' ) || parent::get_items_permissions_check( $request );
	}
	 ...
	 * for the post being checked or if pb_set_items_permissions filter is true.
	 *
	 * @since 5.7.1
	 *
