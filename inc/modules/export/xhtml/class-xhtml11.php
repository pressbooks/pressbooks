	...
	protected function echoFrontMatterGenerator( $book_contents, $metadata ) : \Generator {
		...
		foreach ( $book_contents['front-matter'] as $front_matter ) {
			...
			printf(
				...
				$subclass,
				$slug,
				( $short_title ) ? Sanitize\decode( $short_title ) : wp_strip_all_tags( Sanitize\decode( $front_matter['post_title'] ) ),
				$i,
				Sanitize\decode( $title ),
				$after_title,
				$content,
	...
	protected function echoBackMatterGenerator( $book_contents, $metadata ) : \Generator {
		...
		foreach ( $book_contents['back-matter'] as $back_matter ) {
			...
			printf(
				...
				$subclass,
				$slug,
				( $short_title ) ? Sanitize\decode( $short_title ) : wp_strip_all_tags( Sanitize\decode( $back_matter['post_title'] ) ),
				$i,
				Sanitize\decode( $title ),
				$after_title,
				$content,
