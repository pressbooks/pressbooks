...
<div class="wrap">
	<h1><?php _e( 'Cover Generator', 'pressbooks-cg' ); ?></h1>
	...
	<?php if ( $is_custom_css ) { ?>
		<?php printf( '<div class="notice notice-warning"><p>%s</p></div>', __( 'You are currently using the Custom CSS theme. To generate a cover, you will need to switch back to a base theme temporarily. You can then reapply your Custom CSS theme.', 'pressbooks-cg' ) ); ?>
	<?php } ?>


	<p><?php _e( 'Using our cover generator, you can easily create a beautiful cover for your <strong>print</strong> book that will work with most Print on Demand services, as well as an <strong>ebook</strong> cover that will meet the specifications of Kindle, Apple and other ebook retailers.', 'pressbooks-cg' ); ?></p>
	...
	<h3><?php _e( 'Make Your Cover', 'pressbooks-cg' ); ?></h3>

	<?php if ( $is_custom_css ) { ?>
		<?php printf( '<p><em>%s</em></p>', __( 'You are currently using the Custom CSS theme. To generate a cover, you will need to switch back to a base theme temporarily. You can then reapply your Custom CSS theme.', 'pressbooks-cg' ) ); ?>
	<?php } ?>

	<form class="generate-file pdf" action="<?php echo $generate_form_url; ?>" method="post">
		<input type="hidden" name="format" value="pdf"/>
	</form>
	<input type="button" id="generate-pdf" class="button<?php if ( ! $is_custom_css ) { ?> button-primary<?php } ?>" value="<?php _e( 'Make PDF Cover', 'pressbooks-cg' ); ?>"
			<?php if ( $is_custom_css ) { ?>disabled <?php } ?>/>

	<form class="generate-file jpg" action="<?php echo $generate_form_url; ?>" method="post">
		<input type="hidden" name="format" value="jpg"/>
	</form>
	<input type="button" id="generate-jpg" class="button<?php if ( ! $is_custom_css ) { ?> button-primary<?php } ?>" value="<?php _e( 'Make Ebook Cover', 'pressbooks-cg' ); ?>"
			<?php if ( $is_custom_css ) { ?>disabled <?php } ?>/>
	<h3><?php _e( 'Download', 'pressbooks-cg' ); ?></h3>
	<?php
	$covers = \Pressbooks\Utility\group_exports( $covers_folder );
	if ( empty( $covers ) ) {
		echo '<p>' . _e( 'Sorry! You have to generate some covers first.', 'pressbooks-cg' ) . '</p>';
	} else {
		/* translators: %s date/time */
		printf( '<p>' . __( 'Your latest covers were generated on %s. Hover to download them.', 'pressbooks-cg' ) . '</p>', strftime( '%B %e, %Y at %l:%M %p', array_keys( $covers )[0] ) );
		?>
						...
						<div class="cover-file-container">
							...
							<div class="file-actions">
								...
								<button class="delete" type="submit" name="submit" src="" value="Delete"
										onclick="if ( !confirm('<?php esc_attr_e( 'Are you sure you want to delete this?', 'pressbooks-cg' ); ?>' ) ) { return false }"><span
			...
			<?php if ( ! empty( $covers ) && current_user_can( 'manage_network' ) ) : ?>
				<form class="delete-all" action="<?php echo $delete_all_form_url; ?>" method="post">
					...
					<button class="button" type="submit" name="submit" src="" value="Delete All Covers"
							onclick="if ( !confirm('<?php esc_attr_e( 'Are you sure you want to delete ALL your current covers?', 'pressbooks-cg' ); ?>' ) ) { return false }"><?php _e( 'Delete All Covers', 'pressbooks-cg' ); ?></button>
				</form>
			<?php endif; ?>
		</div>
		<?php
