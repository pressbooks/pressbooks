...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        ...
        <xsl:for-each select="node()">
          <xsl:choose>
            <xsl:when test="(position( )) = 1 and (name(self::node())) = 'p'">
              <xsl:apply-templates select="." mode="first-p" />
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="p" mode="first-p">
      <p>
        <xsl:choose>
          <xsl:when test="@class">
      ...
      <p>
        <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            <xsl:variable name="pclass"><xsl:if test="@class"> <xsl:value-of select="concat(' ', @class)" /></xsl:if></xsl:variable>
            ...
            <xsl:choose>
              ...
              <xsl:when test="preceding-sibling::*[1][local-name() = 'h1'] or preceding-sibling::*[1][local-name() = 'h2'] or preceding-sibling::*[1][local-name() = 'h3'] or preceding-sibling::*[1][local-name() = 'h4'] or preceding-sibling::*[1][local-name() = 'h5'] or preceding-sibling::*[1][local-name() = 'h6'] or preceding-sibling::*[1][local-name() = 'hr']">
                <xsl:attribute name="class">nonindent<xsl:value-of select="$pclass" /></xsl:attribute>
              </xsl:when>
