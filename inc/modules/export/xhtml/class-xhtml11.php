...
*/
            ...
            if (isset($already_done[ $old_src ]) ) {
                ...
                $new_src = maybe_swap_with_bigger($old_src);
            }
            if ($old_src !== $new_src ) {
        ...
        file_put_contents(WP_CONTENT_DIR."/content-after-images-$random.html", $content);
    ...
    protected function html5ToXhtml( $html )
    {
        ...
        $html = HtmLawed::filter($html, $config);
        $rand = time();
        file_put_contents(WP_CONTENT_DIR."/content-after-xhtml-$rand.html", $html);
        return $html;
    }
        ...
        foreach ( $book_contents['part'] as $part ) {
            ...
            foreach ( $part['chapters'] as $chapter ) {
                ...
                if (! $chapter['export'] ) {
                   ...
                   file_put_contents(WP_CONTENT_DIR."/chapter-$chapter_id.html", $chapter_content);
                ...
                $rendered_chapters .= $this->blade->render(
                   ...
                   file_put_contents(WP_CONTENT_DIR."/chapter-$chapter_id-rendered.html", $rendered_chapters);
