...
class Epub extends ExportGenerator {
	...
	protected function createContainer() {
		...
		mkdir( $this->tmpDir . '/EPUB' );
		mkdir( $this->tmpDir . '/EPUB/assets' );
	 ...
	 * @throws \Exception
	 */
	protected function createEPUBGenerator( $book_contents, $metadata ) : \Generator {
	...
	protected function createStylesheet() {
		...
		$path_to_tmp_stylesheet = $this->tmpDir . "/EPUB/{$this->stylesheet}";
	...
	protected function scrapeKneadAndSaveCss( $path_to_original_stylesheet, $path_to_copy_of_stylesheet ) {
		...
		$path_to_epub_assets = $this->tmpDir . '/EPUB/assets';
	...
	protected function renderCover( array $metadata ) {
		...
		$dest_path = $this->tmpDir . '/EPUB/assets/' . $dest_image;
	 ...
	 * @throws \Exception
	 */
	protected function renderToc( $metadata ) {
	...
	protected function createOPF( $book_contents, $metadata ) {
		...
		$html = '';
		foreach ( $this->manifest as $k => $v ) {
			$properties = $this->getProperties( $this->tmpDir . '/EPUB/' . $v['filename'] );
	...
	protected function buildManifestAssetsHtml() {
		...
		$html = '';
		$path_to_assets = $this->tmpDir . '/EPUB/assets';
