...
class Epub extends ExportGenerator
{
    ...
    protected function normalizeCssUrls(string $css, string $scss_dir, string $path_to_epub_assets): string
    {
        ...
        $css = preg_replace_callback(
            $url_regex,
            function ($matches) use ($scss_dir, $path_to_epub_assets) {
            ...
            if (0 === strpos($k, 'part-')) { //Process parts
                ...
                $rendered_items[] = $this->blade->render('export/bullet-toc-part', [
                    ...
                    'item' => [
                        'is_epub' => true,
                        'slug' => $data['href'],
                        'title' => ($this->numbered && $is_visible ? apply_filters(
                            'pb_post_type_label',
                            __('Part', 'pressbooks'),
                            [ 'post_type' => 'part' ]
                        ) . ' ' . \Pressbooks\L10n\romanize($parts_count) . '. ' : '') . $data['title'],
                    ],
                ]);

                $parts_count = $is_visible ? ++$parts_count : $parts_count;

            } elseif (0 === strpos($k, 'front-matter-') || 0 === strpos(
                $k,
                'back-matter-'
            )) { //Process front/back matters
