...
function batch_serve_request($request)
{
    if (! is_array($request['requests'])) {
        return new \WP_Error(
            'rest_invalid_param',
            __('Invalid batch parameter(s).', 'pressbooks'),
            [
                'status' => 400,
            ]
        );
...
function update_part_id($part_id, $post_obj)
{
    ...
    if ($part === null) {
        return new \WP_Error(
            'rest_chapter_part_failed',
            __('Part does not exist', 'pressbooks'),
            [
                'status' => 500,
            ]
        );
    ...
    if ($part->post_type !== 'part') {
        return new \WP_Error(
            'rest_chapter_part_failed',
            __('ID is not a part', 'pressbooks'),
            [
                'status' => 500,
            ]
        );
    ...
    if (false === $ret) {
        return new \WP_Error(
            'rest_chapter_part_failed',
            __('Failed to update chapter part', 'pressbooks'),
            [
                'status' => 500,
            ]
        );
...
function is_enabled()
{
    $enable_network_api = SharingAndPrivacyOptions::getOption('enable_network_api');
