...
/*!********************************************!*\
  !*** ./assets/src/scripts/contributors.js ***!
  \********************************************/
/* global pictureSize */
jQuery(function ($) {
  var minPictureSize = parseInt(pictureSize.min),
  ...
  var mustBeCropped = function mustBeCropped(flexW, flexH, dstW, dstH, imgW, imgH) {
  ...
  var imgSelectOptions = function imgSelectOptions(attachment, controller) {
    ...
    var isBiggerThanMinimum = realHeight > minPictureSize || realWidth > minPictureSize;
    imgSelectOptions = {
      handles: true,
      keys: true,
      instance: true,
  ...
  jQuery(document).ajaxSend(function (event, xhr, settings) {
    if (settings.data.indexOf('action=add-tag') >= 0) {
      ...
      var dataEncoded = new URLSearchParams(settings.data);
  ...
  jQuery('#plupload-browse-button').on('click', function (e) {
    // Cropper
    var Cropp = wp.media.controller.CustomizeImageCropper.extend({
      /**
      ...
      doCrop: function doCrop(attachment) {
        ...
        var minSize = parseInt(minPictureSize);
    ...
    pictureLibrary.on('insert', function () {
      var attachment = pictureLibrary.state().get('selection').first().toJSON();
    ...
    pictureLibrary.on('select', function () {
      ...
      if (attachment.width < minPictureSize || attachment.height < minPictureSize) {
        var htmlError = '<div class="media-uploader-status errors">' + '<div class="upload-errors"><div class="upload-error">\n' + '<span class="upload-error-filename">Your image is too small.</span>' + '<span class="upload-error-message">' + 'The image must be ' + minPictureSize + ' by ' + minPictureSize + ' pixels. Your image is ' + attachment.width + ' by ' + attachment.height + ' pixels.</span></div></div></div>';
        jQuery('.media-sidebar').html(htmlError);
        return;
      }
