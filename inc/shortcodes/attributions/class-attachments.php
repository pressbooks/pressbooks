	...
	public function attributionsContent( $attributions ) {
		...
		if ( $attributions ) {
			...
			if ( ! empty( $media_attributions ) ) {
				$html = sprintf(
					'<div class="media-attributions clear" prefix:cc="http://creativecommons.org/ns#" prefix:dc="http://purl.org/dc/terms/"><h2>' . __( 'Media Attributions', 'pressbooks' ) . '</h2><ul>%s</ul></div>',
					$media_attributions
				);
			}
		}
