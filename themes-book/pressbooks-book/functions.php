...
function pressbooks_book_info_page () {
	...
	if ( is_front_page() ) {
		wp_enqueue_style( 'pressbooks-book-info', PB_PLUGIN_URL . 'themes-book/' . get_template() . '/css/book-info.css', array(), '20130713', 'all' );
		...
		wp_enqueue_script( 'columnizer-load', PB_PLUGIN_URL . 'themes-book/' . get_template() . '/js/columnizer-load.js', array( 'jquery', 'columnizer' ), '20130819', false );

		// Sharrre
		wp_enqueue_script( 'sharrre', PB_PLUGIN_URL . 'symbionts/jquery/sharrre/jquery.sharrre-1.3.4.min.js', array( 'jquery' ), '20130712', false );
		wp_enqueue_script( 'sharrre-load', PB_PLUGIN_URL . 'themes-book/' . get_template() . '/js/sharrre-load.js', array( 'jquery', 'sharrre' ), '20130712', false );
...
function pb_enqueue_scripts() {
	...
	if (! is_front_page() ) {
		wp_enqueue_script( 'pressbooks-script', PB_PLUGIN_URL . 'themes-book/' . get_template() . "/js/script.js", array( 'jquery' ), '1.0', false );
	}
	wp_enqueue_script( 'keyboard-nav', PB_PLUGIN_URL . 'themes-book/' . get_template() . '/js/keyboard-nav.js', array( 'jquery' ), '20130306', true );

	if ( is_single() ) {
		wp_enqueue_script( 'pb-pop-out-toc', PB_PLUGIN_URL . 'themes-book/' . get_template() . '/js/pop-out.js', array( 'jquery' ), '1.0', false );
	}

	$options = get_option( 'pressbooks_theme_options_web' );
	if ( @$options['toc_collapse'] ) {
		wp_enqueue_script( 'pressbooks_toc_collapse',	PB_PLUGIN_URL . 'themes-book/' . get_template() . '/js/toc_collapse.js', array( 'jquery' ) );
		wp_enqueue_style( 'dashicons' );
	}
	if ( @$options['accessibility_fontsize'] ) {
		wp_enqueue_script( 'pressbooks-accessibility', PB_PLUGIN_URL . 'themes-book/' . get_template() . '/js/a11y.js', array( 'jquery' ) );
		wp_register_style( 'pressbooks-accessibility-toolbar', PB_PLUGIN_URL . 'themes-book/' . get_template() . '/css/a11y.css', array(), null, 'screen' );
