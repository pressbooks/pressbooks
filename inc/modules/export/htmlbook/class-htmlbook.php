		...
		foreach ( $book_contents['front-matter'] as $front_matter ) {
			...
			$fm->setAttributes(
				[
					'class' => "front-matter {$subclass}",
					'id' => "front-matter-{$front_matter['post_name']}",
					'title' => $short_title ? $short_title : wp_strip_all_tags( html_entity_decode( $front_matter['post_title'] ) ),
				]
			);
		...
		foreach ( $book_contents['part'] as $part ) {
			...
			foreach ( $part['chapters'] as $chapter ) {
				...
				$my_chapter->appendAttributes(
					[
						'id' => $slug,
						'title' => $short_title ? $short_title : wp_strip_all_tags( html_entity_decode( $chapter['post_title'] ) ),
					]
				);
		...
		foreach ( $book_contents['back-matter'] as $back_matter ) {
			...
			$bm->setAttributes(
				[
					...
					'title' => $short_title ? $short_title : wp_strip_all_tags( html_entity_decode( $back_matter['post_title'] ) ),
				]
			);
